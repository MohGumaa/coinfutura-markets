...
const BlogSearch = ({ isSearchOpen }: BlogSearchProps) => {
  ...
  const getFeaturedImage = useCallback((post: WordPressPost): string => {
    return post._embedded?.["wp:featuredmedia"]?.[0]?.media_details?.sizes?.thumbnail?.source_url || "/icons/cfu.jpg"
  }, [])

  const moreResultsUrl = useMemo(() => {
    return `${env.mainDomain}?s=${encodeURIComponent(searchTerm)}`
  }, [searchTerm])
  ...
  }, [showResults, resetSearch])

  // Handle Escape and Scroll
  useEffect(() => {
    if (!showResults) return
    ...
    const handleScroll = () => resetSearch()
            ...
            {results.map((post) => (
              ...
              >
                <Image 
                  ...
                  width={56}
                  height={48}
                  className="w-14 h-12 object-cover rounded wp-post-image flex-shrink-0"
                />
                <h4 className="font-medium article-title group-hover:text-blue-600 dark:group-hover:text-blue-400 line-clamp-2 flex-1">
                  {post.title.rendered.replace(/<[^>]*>/g, "")}
                </h4>
              </a>
            ))}
          </div>
          <div className="flex justify-center p-3 border-t border-gray-950/5 dark:border-white/10">
            <a 
              href={moreResultsUrl}
